...
the local `discovery.json` and never asks Youfone what exists - `cmd_discover`
...
Ported from apps/simyo/tests/test_scheduled_run.py.
"""
...
def test_the_schedulers_own_argv_reaches_discover(tmp_path, monkeypatch):
    ...
    cmd = schedule.build_command(
        {"app": "youfone", "account": "primary", "config": "x"},
        APP_DIR, APP_DIR / "youfone_docs.py")
    flags = [f for f in cmd[2:] if f != "x" and f != "--config"]

    app, args = _app(tmp_path, flags)
    seen = _trace(app, monkeypatch)
    assert youfone_docs._dispatch(app, args) == 0
...
def test_the_schedulers_own_argv_passes_the_unattended_guard(tmp_path):
    ...
    expression and comparing it to itself. Routing a config path that is
    never created makes "admitted" and "refused" tell apart without a
    browser or a provider: a refused combination returns 2 before App(args)
    is ever built, so the missing config is never touched, while an admitted
    one falls through into App(args) and dies on load_config's own
    SystemExit instead of returning 2.
    """
    schedule = _schedule()
    cmd = schedule.build_command(
        {"app": "youfone", "account": "primary", "config": "x"},
    ...
    with pytest.raises(SystemExit):
        youfone_docs.main(flags + ["--config", str(missing_config)])
...
def test_run_all_reaches_discover(tmp_path, monkeypatch):
    ...
    seen = _trace(app, monkeypatch)
    youfone_docs._dispatch(app, args)
    assert "discover" in seen
...
def test_resume_with_nothing_to_do_still_records_a_live_session(
    ...
    monkeypatch.setattr(youfone_docs.site, "detect_security_challenge",
                        lambda page: "")
    monkeypatch.setattr(youfone_docs.site, "looks_signed_out", lambda page: False)
