...
In the Docker layout, "no Simyo tab open" is the single most likely reason a
scheduled run finds nothing: the human simply hasn't signed in yet today.
...
def _app(tmp_path, unattended: bool) -> simyo_docs.App:
...
def test_unattended_with_no_signed_in_tab_parks(tmp_path, monkeypatch):
    monkeypatch.setattr(simyo_docs.site, "find_signed_in_page", lambda ctx: None)
