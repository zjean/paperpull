...
def _app(tmp_path, unattended: bool = False) -> simyo_docs.App:
...
def test_a_park_mid_run_stops_the_run_instead_of_counting_as_a_failure(
    ...
    def die(page, doc, filename):
        raise simyo_docs.Parked("signed out")

    monkeypatch.setattr(app, "download_one", die)
    docs = [simyo_docs.Document(title="Factuur juni", category="Statement",
                                date="2026-06-01"),
            simyo_docs.Document(title="Factuur juli", category="Statement",
                                date="2026-07-01")]
