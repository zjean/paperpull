"""Simyo document classification: the titles are Dutch, the categories are not.
...
that lands in the filename is the word Simyo itself uses.
"""
import sys
...
import storage  # binds Simyo's AppSpec (the rules file is found through it)
...
def test_a_usage_specification_is_a_statement_too():
    """Not fetched today, but Simyo calls the per-call breakdown a
    'specificatie' and it is a statement if one ever arrives."""
    cat, summary, _ = doc_types.classify_document("Specificatie juni 2026", RULES)
...
def test_dutch_noise_is_skipped():
    ...
    assert not doc_types.should_skip("Factuur 97102597", RULES)
...
def test_an_invoice_routes_to_the_statements_folder(tmp_path):
    ...
    cat, _, _ = doc_types.classify_document("Factuur 97102597", RULES)
